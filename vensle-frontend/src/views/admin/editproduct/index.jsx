...
const EditProduct = () => {
      ...
      <div key={index} className="relative">
        ...
        {!isMainPreview(index) && (
          <button
            ...
            className="absolute top-0 right-0 flex cursor-pointer items-center justify-center rounded-full bg-red-500 text-white"
        ...
        {!isMainPreview(index) && (
          <button
            ...
            className="absolute bottom-0 cursor-pointer rounded-lg bg-green-500 p-2 text-white opacity-80 hover:opacity-100"
  ...
  const handleRemoveOldImage = (e, imageId, imageName) => {
    ...
    const updatedImages = formData.oldImages.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        { ...formData, removedImages, mainImageIndex: proofImageIndex },
        {
          headers: {
  ...
  useEffect(() => {
    const getUserCountry = async () => {
      try {
        ...
        const response = await axios.get(
          "https://ipinfo.io/json?token=09389931bcf565"
        );
