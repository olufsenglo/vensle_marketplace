...
const formattedTotalPrice = formatPrice(totalPrice);
  ...
  const handleToken = async (token) => {
    ...
    //}
      ...
      const response = await axios.post(`${baseURL}/api/v1/payment`, { product_ids: productIds }, {
        headers: {
        ...
        <p>Your cart is empty.</p>
			...
			src={getCartDisplayImage(item)} alt={item.name} />
                    ...
                    <div className="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                        <button onClick={() => handleRemoveFromCart(item.id)} type="button" className="flex rounded p-2 text-center text-gray-500 transition-all duration-200 ease-in-out focus:shadow hover:text-gray-900">
